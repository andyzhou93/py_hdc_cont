...
iter = 0
population = np.zeros((NPop,3),dtype='int')
for N in range(NPop):
    population[N,:] = np.random.choice(NQuantPool,3)
...
#         print(population[N], fitness[N])
    ...
    for i in range(NPop-NNew,NPop):
        population[i,:] = np.random.choice(NQuantPool,3)
...
with open('imu_encoding_random_genetic.pickle', 'wb') as f:
